...
public class Mage : MonoBehaviour
{
    ...
    public Transform projectileSpawnPoint; // Point where the projectile will spawn
    ...
    private IEnumerator ShootProjectile()
    {
        ...
        float distToPlayer = Vector2.Distance(posicionJugador, transform.position);
        float distToInvader = Vector2.Distance(posicionInvasor, transform.position);

        // Determine the target position based on the nearest one
        Vector2 targetPosition = distToPlayer < distToInvader ? posicionJugador : posicionInvasor;

        // Create a projectile at the spawn point
        GameObject projectile = Instantiate(projectilePrefab, projectileSpawnPoint.position, Quaternion.identity);

        // Set the projectile's direction towards the nearest target
    ...
    private void InitializeComponents()
    {
        ...
        moveSpeed = Random.Range(moveSpeed - 1.25f, moveSpeed);
    }
